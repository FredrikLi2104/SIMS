...
@section('content')
    ...
    @endif
    <organisation-review locale="{{ App::currentLocale() }}" :review-statuses="{{ Js::from($reviewStatuses) }}" action-id="{{ $actionId }}" :plans="{{ Js::from($plans) }}" :auditor-statements="{{ Js::from($auditorStatements) }}" :org="{{session()->get('selected_org')['id']}}" />
@endsection
